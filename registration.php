<?php
// Backend includes (keep logic intact)
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
  ...
  if(strlen($username) < 4){ $error['username'] = 'Username needs to be longer'; }
  if($username === ''){ $error['username'] = 'Username cannot be empty'; }
  if(username_exists($username)){ $error['username'] = 'Username already exists, pick another'; }

  if($email === ''){ $error['email'] = 'Email cannot be empty'; }
  if(email_exists($email)){ $error['email'] = 'Email already exists, <a href="index.php">Please login</a>'; }

  if($password === ''){ $error['password'] = 'Password cannot be empty'; }

  // Clear empty errors
  foreach($error as $k=>$v){ if(empty($v)) unset($error[$k]); }

  if(empty($error)){
    register_user($username, $email, $password);
    // Auto-login
    login_user($username, $password);
    // login_user should redirect; if not, fallback:
    header('Location: '.(defined('BASE_URL')?BASE_URL:'./')); exit;
  }
}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .card{width:min(560px,92vw);border:1px solid var(--stroke);background:var(--glass);border-radius:24px;overflow:hidden;box-shadow:var(--shadow)}
    .card .cover{height:140px;background:radial-gradient(120% 120% at 10% 0%, rgba(83,41,237,.55), transparent 60%), radial-gradient(120% 120% at 110% 100%, rgba(38,14,208,.55), transparent 60%)}
    .card .body{padding:22px}
    h1{font-family:'Playfair Display',serif;font-size:30px;margin:0 0 8px}
    ...
    .error{margin-top:6px;color:#FF99A1;font-size:13px}
...
<body>
 ...
 <div class="nav">
  ...
  <main class="wrap vh">
    <div class="card">
      ...
      <div class="body">
        ...
        <p class="sub">Join Niterria — luxury tech, without the noise.</p>
        <form class="form" role="form" action="registration.php" method="post" id="login-form" autocomplete="off">
          <!-- Username -->
          <div class="field">
            <input type="text" name="username" id="username" placeholder="Enter desired username" autocomplete="on" value="<?= h($username) ?>" required>
          ...
          <?php if(isset($error['username'])): ?><div class="error"><?= $error['username'] ?></div><?php endif; ?>

          <!-- Email -->
          <div class="field">
            <input type="email" name="email" id="email" placeholder="somebody@example.com" autocomplete="on" value="<?= h($email) ?>" required>
            <svg class="icon" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 6h16v12H4V6Zm0 0 8 6 8-6" stroke="#cbd5e1" stroke-width="2"/></svg>
          </div>
          <?php if(isset($error['email'])): ?><div class="error"><?= $error['email'] ?></div><?php endif; ?>

          <!-- Password -->
          <div class="field">
            <input type="password" name="password" id="key" placeholder="Create a password" required>
          ...
          <?php if(isset($error['password'])): ?><div class="error"><?= $error['password'] ?></div><?php endif; ?>

          <div class="actions">
            <div></div>
            <button class="btn primary" type="submit" name="resgister" id="btn-login">Register</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <script>
    ...
    window.addEventListener('scroll', () => { if (window.scrollY > 240) toTop.classList.add('show'); else toTop.classList.remove('show'); });
